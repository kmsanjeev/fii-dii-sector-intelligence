...
# Phase 12C — forward return scores from realized bhavcopy returns
...
class FeatureEngineeringEngine:
    ...
    def _build_matrix(self) -> pd.DataFrame:
        ...
        bull = self._add_technical_patterns(bull)

        # Phase 12C — forward return score (trained on realized returns, not rule-based)
        bull = self._add_forward_return_score(bull)
    ...
    def _add_technical_patterns(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Phase 12B: RSI(14), MACD(12,26,9), Bollinger %B (20,2), ADX(14).
        Source: data/intelligence/technical_pattern_features.csv
        Generated by engines/ml/technical_feature_engine.py (run before feature_engineering).
        """
        if not TECH_PATTERNS.exists():
            logger.warning(
                "[FeatureEng 12B] technical_pattern_features.csv missing -- "
                "run engines.ml.technical_feature_engine first"
            )
            return df
        try:
            tp = pd.read_csv(
            ...
            # Clip continuous features to sensible ranges
            tp["rsi_14"]    = pd.to_numeric(tp["rsi_14"],    errors="coerce").clip(0, 100)
            tp["macd_hist"] = pd.to_numeric(tp["macd_hist"], errors="coerce").clip(-50, 50)
            tp["bb_pct_b"]  = pd.to_numeric(tp["bb_pct_b"],  errors="coerce").clip(-0.5, 1.5)
            tp["bb_squeeze"] = pd.to_numeric(tp["bb_squeeze"], errors="coerce").clip(0, 50)
            tp["adx_14"]    = pd.to_numeric(tp["adx_14"],    errors="coerce").clip(0, 100)
            logger.info("[FeatureEng 12B] Technical patterns: %d symbols", tp["symbol"].nunique())
            return df.merge(tp, on="symbol", how="left")
        except Exception as e:
            logger.warning("[FeatureEng 12B] Technical patterns failed: %s", e)
            return df

    def _add_forward_return_score(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Phase 12C: probability score from the forward-return-trained XGBoost model.
        Source: data/intelligence/ml_forward_return_scores.csv
        Generated by engines/ml/forward_return_model.py.
        Provides a true supervised signal (trained on realized returns) orthogonal
        to the rule-based label_enc derived from Phase 8B scores.
        """
        if not FWD_RETURN_SCORES.exists():
            logger.warning(
                "[FeatureEng 12C] ml_forward_return_scores.csv missing -- "
                "run engines.ml.forward_return_model first"
            )
            return df
        try:
            frs = pd.read_csv(FWD_RETURN_SCORES, usecols=["symbol", "forward_return_score"])
            frs["symbol"] = frs["symbol"].str.strip().str.upper()
            frs["forward_return_score"] = pd.to_numeric(
                frs["forward_return_score"], errors="coerce"
            ).clip(0, 100)
            logger.info("[FeatureEng 12C] Forward return scores: %d symbols", frs["symbol"].nunique())
            return df.merge(frs, on="symbol", how="left")
        except Exception as e:
            logger.warning("[FeatureEng 12C] Forward return score failed: %s", e)
            return df

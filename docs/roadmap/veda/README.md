...
- `VEDA-EMP-MARRIAGE-010` completed its ten-case sanity pilot with primary `NO_SEPARATION` after independent matched-control scoring. `VEDA-EMP-050-REPLICATION-SIGNAL-002` completed the authorized marriage 50-case lane; its independent new 25 did not reproduce the original separation, so the bounded status is `NO_SEPARATION_AT_EMP025` / `NON_REPLICATING_PILOT_SIGNAL`. `VEDA-SIGNAL-002` froze the narrow research-only `VEDA-SIGNAL-PROGENY-OCCURRENCE-001` before outcome inspection; signal-specific cases remain 0 and the pilot is not ready. General EMP-050 remains at 25/50; education, career commencement, relocation and property remain `SOURCE_PARTIAL`. No production, RAG, Approved Core or doctrine change occurred.

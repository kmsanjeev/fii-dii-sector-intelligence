...
- `VEDA-EMP-MARRIAGE-010` completed its ten-case sanity pilot with primary `NO_SEPARATION` after independent matched-control scoring; the unsealed holdout and combined view are descriptive only. `VEDA-EMP-MARRIAGE-025` completed a separate 25-case replication whose holdout did not reproduce design/validation separation. EMP-050 remains at 25/50; no production, RAG, Approved Core or doctrine change occurred.

...
- `VEDA-CALC-ASHTAKAVARGA-INVARIANT-RX-001` completed with `PASS_WITH_CONDITION` and decision `ASHTAKAVARGA_CURRENT_CANONICAL_IS_INVALID_HYBRID`: the hash-verified BPHS matrix and independent source reconstruction compute 336 planetary, 49 Lagna and 385 combined; the modern 337/386 witness is not direct BPHS/KNR numerical proof. V1 remains preserved as superseded, and a non-production V2 candidate records 336/49/385. No production BAV/SAV, reductions, P018, prediction, ML, RAG, D20 or Approved Core state changed. Evidence is in `docs/current-state/calc-ashtakavarga-invariant-rx-001/`.

- `VEDA-CALC-ASHTAKAVARGA-REMEDIATION-001` remains in a governed blocked state: its preserved V1 contract requires 337 ordinary planetary SAV and 386 combined display, while its verified 768-cell source matrix computes 336 and 385. The invariant reconciliation has resolved this as an invalid hybrid; a separate production contract/engine repair is still unauthorized. No production BAV/SAV, P018, prediction, ML, RAG, D20 or Approved Core state changed. Evidence is in `docs/current-state/calc-ashtakavarga-remediation-001/`.

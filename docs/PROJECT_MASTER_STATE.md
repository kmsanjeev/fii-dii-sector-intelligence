...
- `VEDA-CALC-ASHTAKAVARGA-CONTRACT-RX2-001` completed with `ASHTAKAVARGA_V2_CONTRACT_ACCEPTED_WITH_CONDITION`: canonical source contract `ASHTAKAVARGA_RAW_BPHS_PRIMARY_V2` is frozen at hash `084E19B2D61880066A503E1CED38810CA9D51962354A9520DD2E5E5946279A62`, based on the frozen BPHS matrix hash `0B7A869F3A3682A3BFFADA28E82AC23DC96EFE7E6FF3763997317C5050EE159D`, with 336 planetary / 49 Lagna / 385 combined semantics. Runtime remains `IMPLEMENTED_UNVALIDATED`; `VEDA-CALC-ASHTAKAVARGA-REMEDIATION-RX2-001` is authorized but not started. No production code, RAG, prediction, ML or Approved Core state changed. Evidence is in `docs/current-state/calc-ashtakavarga-contract-rx2-001/`.
...
- `VEDA-CALC-ASHTAKAVARGA-REMEDIATION-001` remains in a governed blocked historical state: its preserved V1 contract requires 337 ordinary planetary SAV and 386 combined display, while its verified 768-cell source matrix computes 336 and 385. The invariant reconciliation resolved this as an invalid hybrid; its replacement `VEDA-CALC-ASHTAKAVARGA-REMEDIATION-RX2-001` is now authorized but not started against canonical V2. No production BAV/SAV, P018, prediction, ML, RAG, D20 or Approved Core state changed. Evidence is in `docs/current-state/calc-ashtakavarga-remediation-001/`.
...
- VEDA-CALC-SIDEREAL-ASC-TZ-001 completed with `PASS_WITH_CONDITION`: official IMD/PAC IAE 2026 evidence provides a bounded Lahiri/Nirayana convention check, while full all-body independent sidereal authority remains partial. A 120-case independent tropical Ascendant corpus passes 120/120 within 0.05°, inherited boundary cases require explicit policy, and a 64-case timezone corpus records tzdata version, historical LMT, DST gaps and folds without false precision. The stale OpenAPI snapshot was regenerated canonically to the live 140-path/153-operation contract. The unified RAG snapshots changed only through deterministic regeneration of current governed capability records; no new RAG store, trust-zone promotion, production chart, D20 interpretation, prediction, ML or PRED-M4 activation occurred. Evidence is in `docs/current-state/calc-sidereal-asc-tz-001/`.
 - VEDA-CALC-JYOTISHA-CORE-001 completed with `PASS_WITH_CONDITION`: D1/Rashi, the implemented Varga inventory, Vimshottari/Antardasha, deterministic rule-engine traceability, and internal Ashtakavarga invariants were audited with independent fixtures. The independent Varga grid covers 2,220 cases; D1, Dasha, rule and inherited Silver/Stress results pass. Ashtakavarga calculation authority remains unvalidated, D20 destination mapping remains evidence-qualified, and the full suite timed out at 15 minutes without being treated as a pass. No production, predictive, ML, PRED-M4, RAG or Approved Core change occurred. Evidence is in `docs/current-state/calc-jyotisha-core-001/`.

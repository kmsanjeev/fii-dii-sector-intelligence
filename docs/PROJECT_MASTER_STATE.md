...
- VEDA-EMP-050-SIGNAL-SEARCH-001 remains the separate general 25/50 acquisition lane. VEDA-EMP-050-REPLICATION-SIGNAL-002 completed the marriage 50-case lane: the independent new 25 did not reproduce the original separation, yielding `NO_SEPARATION_AT_EMP025` / `NON_REPLICATING_PILOT_SIGNAL`. VEDA-SIGNAL-002 then froze the narrow research-only `VEDA-SIGNAL-PROGENY-OCCURRENCE-001` before outcome inspection; signal-specific cases remain 0 and no pilot has started. Education, career commencement, relocation and property remain `SOURCE_PARTIAL`; the EMP-025 holdout remains sealed.

...
- VEDA-EMP-050-SIGNAL-SEARCH-001 remains the separate general 25/50 acquisition lane. VEDA-EMP-050-REPLICATION-SIGNAL-002 completed the marriage 50-case lane: the independent new 25 did not reproduce the original separation, yielding `NO_SEPARATION_AT_EMP025` / `NON_REPLICATING_PILOT_SIGNAL`. VEDA-EMP-PROGENY-010 acquired and chart-froze ten public childbirth cases and completed the frozen progeny sanity pilot with primary `NO_SEPARATION`; the signal remains research-only and immutable. Education, career commencement, relocation and property remain `SOURCE_PARTIAL`; the EMP-025 holdout remains sealed.

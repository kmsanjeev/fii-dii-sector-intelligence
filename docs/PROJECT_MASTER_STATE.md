...
- VEDA-CALC-ASHTAKAVARGA-CROSSSOURCE-RX-001 completed with `PASS_WITH_CONDITION`: BPHS and Phaladeepika were independently compared through source-preserving 768-cell witness matrices, five witnesses were registered, and 96 deterministic source-variant charts were evaluated without importing production tables. Phaladeepika retains an explicit Varahamihira variant and seven-planet SAV statement; complete Bṛhat Jātaka, Saravali and Jataka Parijata numerical normalization remains unavailable. Final decision is `ASHTAKAVARGA_TEXTUAL_AMBIGUITY_BLOCKS_REMEDIATION`; no production BAV/SAV, P018, D20, P032, prediction, ML, RAG or Approved Core change occurred. Evidence is in `docs/current-state/calc-ashtakavarga-crosssource-rx-001/`.
- VEDA-CALC-ASHTAKAVARGA-NORMALIZATION-RX2-001 completed with `PASS_WITH_CONDITION`: lawful K.N. Rao/BVB/Journal/Google Books lineage checks and a page-inspected modern implementation witness govern a BPHS-primary raw contract specification. Planetary SAV is 337; 386 is an optional Lagna-combined display; Lagna contributor/target, self, polarity, node exclusion and raw/reduced separation are explicit. Bṛhat Jātaka and Saravali numerical normalization remain unresolved and Jataka Parijata remains reference-not-verified; reductions remain deferred. Production Ashtakavarga mathematics, P018, D20, P032, prediction, ML, RAG and Approved Core are unchanged; production remediation is not authorized. Evidence is in `docs/current-state/calc-ashtakavarga-normalization-rx2-001/`.

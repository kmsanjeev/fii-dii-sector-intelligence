# FUNDAMENTAL INTELLIGENCE

## Current governed evidence boundary (2026-08-21)

The additive `fundamental-evidence-1.0` projection is the current provenance
boundary for stock and cross-layer fundamental context. It keeps quarterly,
TTM, YOY and aggregate periods distinct; separates period end, filing date and
local retrieval time; refuses incomplete TTM annualisation; preserves negative
and missing values; and marks financial-sector metric limitations. Legacy
valuation scores remain compatibility context only. PE/PB do not establish a
fair-value conclusion, and the legacy `roe_pct` column is not treated as ROE.

Fundamental evidence is descriptive and source-qualified. It is not a
recommendation, prediction, ML feature authorization or PRED-M4 output.

## Project

FII/DII Capital Flow Intelligence Platform

---

# Module Overview

Fundamental Intelligence explains why capital is flowing into or out of a company.

While:

Institutional Intelligence

Sector Intelligence

Theme Intelligence

Stock Intelligence

identify where money is moving,

Fundamental Intelligence identifies the underlying reasons.

---

# Capital Flow Framework

```text
Market
    ↓
Sector
    ↓
Theme
    ↓
Stock
    ↓
Fundamental Reason
```

This module provides the final layer of conviction.

---

# Objective

Answer the following questions:

1. Why is money flowing into a stock?
2. Why is money exiting a stock?
3. What changed fundamentally?
4. Is growth improving or deteriorating?
5. Is management confidence increasing?
6. Is business momentum accelerating?
7. Is the market underestimating future potential?

---

# Why Fundamental Intelligence Matters

Price movement alone is insufficient.

A stock may rise because of:

* Results
* New Orders
* Capacity Expansion
* Management Guidance
* Shareholding Changes
* Regulatory Changes
* Industry Tailwinds

The objective is to identify these drivers before they become consensus.

---

# Intelligence Philosophy

Price reveals behavior.

Fundamentals explain behavior.

---

# Data Sources

---

## Corporate Actions

Provides:

* Bonus
* Split
* Dividend
* Rights Issue
* Buyback

---

## Quarterly Results

Provides:

* Revenue
* EBITDA
* PAT
* Margins
* Growth Rates

---

## Annual Reports

Provides:

* Business Overview
* Strategy
* Risks
* Management Discussion

---

## Concall Transcripts

Provides:

* Management Commentary
* Guidance
* Growth Expectations
* Demand Outlook

---

## AGM Documents

Provides:

* Strategic Updates
* Expansion Plans
* Future Roadmap

---

## Investor Presentations

Provides:

* Business Updates
* Order Book
* Growth Drivers

---

## Shareholding Data

Provides:

* Promoter Holdings
* FII Holdings
* DII Holdings
* Public Holdings

---

## Order Book Data

Provides:

* New Orders
* Order Pipeline
* Execution Visibility

---

## Corporate Announcements

Provides:

* Business Updates
* Contracts
* Partnerships
* Approvals

---

# Core Datasets

---

## Dataset 01

Corporate Actions

Location

```text
data/NSE/corporate_actions/
```

---

## Dataset 02

Quarterly Results

Location

```text
data/NSE/results/
```

---

## Dataset 03

Shareholding

Location

```text
data/NSE/shareholding/
```

---

## Dataset 04

Corporate Announcements

Location

```text
data/NSE/announcements/
```

---

## Dataset 05

Fundamental Intelligence

Location

```text
data/intelligence/fundamental_intelligence.csv
```

---

# Intelligence Framework

Fundamental Intelligence combines:

Business Growth

*

Financial Growth

*

Management Quality

*

Capital Allocation

*

Shareholding Trends

*

Order Visibility

---

# Engines

---

## Engine 01

Results Intelligence Engine

---

### Purpose

Analyze quarterly performance.

---

### Metrics

Revenue Growth

EBITDA Growth

PAT Growth

Margin Expansion

Margin Compression

---

### Outputs

Results Score

Growth Score

Quality Score

---

### Status

Planned

---

## Engine 02

Management Intelligence Engine

---

### Purpose

Analyze management communication.

---

### Sources

Concalls

AGMs

Investor Presentations

---

### Outputs

Confidence Score

Guidance Score

Execution Score

---

### Status

Planned

---

## Engine 03

Shareholding Intelligence Engine

---

### Purpose

Analyze ownership changes.

---

### Inputs

Promoter

FII

DII

Public

---

### Outputs

Accumulation Signals

Distribution Signals

Ownership Trends

---

### Status

Planned

---

## Engine 04

Corporate Actions Intelligence Engine

---

### Purpose

Analyze actions affecting shareholder value.

---

### Examples

Bonus

Split

Dividend

Buyback

Rights Issue

---

### Outputs

Corporate Action Score

Shareholder Alignment Score

---

### Status

Planned

---

## Engine 05

Order Book Intelligence Engine

---

### Purpose

Track future revenue visibility.

---

### Outputs

Order Book Score

Execution Score

Growth Visibility Score

---

### Status

Planned

---

## Engine 06

Growth Intelligence Engine

---

### Purpose

Measure business momentum.

---

### Outputs

Revenue Growth Score

Profit Growth Score

Earnings Momentum Score

---

### Status

Planned

---

## Engine 07

Quality Intelligence Engine

---

### Purpose

Evaluate business quality.

---

### Outputs

Quality Score

Governance Score

Consistency Score

---

### Status

Planned

---

## Engine 08

Valuation Intelligence Engine

---

### Purpose

Compare valuation with growth.

---

### Outputs

Valuation Score

Relative Valuation Score

Opportunity Score

---

### Status

Planned

---

# Future AI Engines

---

## AI Concall Analyst

Purpose

Analyze conference calls.

---

Outputs

Positive Signals

Negative Signals

Management Confidence

Guidance Changes

---

## AI Annual Report Analyst

Purpose

Analyze annual reports.

---

Outputs

Strategic Changes

Risk Factors

Growth Drivers

Management Commentary

---

## AI Order Book Analyst

Purpose

Track business momentum.

---

Outputs

Order Flow Trends

Revenue Visibility

Growth Outlook

---

# Opportunity Framework

Highest conviction opportunities occur when:

```text
Market Alignment
+
Sector Alignment
+
Theme Alignment
+
Stock Alignment
+
Fundamental Confirmation
```

---

# Example

Market

Accumulation

↓

Sector

Defence

↓

Theme

Defence Electronics

↓

Stock

BEL

↓

Fundamentals

Order Book Expansion

Margin Expansion

Management Confidence

↓

High Conviction Opportunity

---

# Dashboard Integration

---

## Fundamental Dashboard

Future Components

Results

Growth

Margins

Shareholding

Order Books

Corporate Actions

Valuation

---

# Infographic Integration

---

## Growth Maps

---

## Order Book Maps

---

## Shareholding Maps

---

## Corporate Action Maps

---

## Earnings Maps

---

# AI Integration

---

## AI Fundamental Analyst

Future Agent

---

### Questions Answered

Why is this stock moving?

What changed fundamentally?

What are management saying?

How strong is the order book?

How strong is future growth?

---

### Outputs

Fundamental Reports

Research Reports

Opportunity Reports

Risk Reports

---

# Relationship To Stock Intelligence

Stock Intelligence identifies:

What is moving.

---

Fundamental Intelligence identifies:

Why it is moving.

---

# Success Criteria

The module successfully identifies:

Business Improvement

Business Deterioration

Management Confidence

Growth Acceleration

Ownership Changes

Future Earnings Potential

before broad market recognition.

---

# Current Completion

Estimated Completion: 100% (Phases 15, 15B, 16 COMPLETE — 2026-07-09)

Completed Engines:
- financial_results_engine.py (Phase 15A): 4181 rows NSE XBRL P&L, 2084 symbols
- valuation_engine.py (Phase 15B): P/E, ROE, valuation_label scoring
- shareholding_engine.py (Phase 15C): quarterly_shp.csv, 8 quarters, 76170 rows
- extended_financials_engine.py (Phase 15B ext): OPM%, ROCE%, Book Value/share, Sales CAGR
- holding_trend_engine.py (Phase 16): QoQ deltas, 7 conviction signals
- announcement_fetcher.py (Phase 16): 527 board announcements, 8-type classification
- management_sentiment_engine.py (Phase 16): rule-based + LLM tone score, 471 symbols

Remaining (future):
- Order Book Intelligence (requires integration with company-level order data)
- AI Concall Analyst (placeholder in Phase 16 architecture — not yet triggered)

---

# Long-Term Vision

Create the most comprehensive fundamental intelligence system capable of explaining:

Why money is flowing,

What changed,

How management is behaving,

How business momentum is evolving,

and whether future growth justifies current valuations.

Fundamental Intelligence serves as the explanation layer of the Capital Flow framework and provides the conviction required for long-term investment decisions.
